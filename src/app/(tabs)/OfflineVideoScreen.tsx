...
const VideoApp = () => {
  ...
  const [sortBy, setSortBy] = useState<
    "title" | "author" | "duration" | "date" | "views"
  >("title");
  ...
  const handleDownload = useCallback(
    async (item: Video) => {
      ...
      if (storageInfo.free < 0.1) {
        Alert.alert(
          "Low Storage Space",
          `You have ${storageInfo.free.toFixed(
            1
          )}GB free space. Please free up some space before downloading.`,
          [
            { text: "Cancel", style: "cancel" },
          ...
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: async () => {
              try {
                ...
                Alert.alert(
                  "Download Started",
                  `"${item.title}" is being downloaded.`
                );
              } catch (error) {
                Alert.alert(
                  "Download Failed",
    ...
    (item: DownloadedVideo) => {
      Alert.alert(
        ...
        `Are you sure you want to delete "${
          item.title
        }"? This will free up ${item.fileSize || "some"} storage space.`,
        [
          { text: "Cancel", style: "cancel" },
          {
            text: "Delete",
            ...
            onPress: async () => {
              try {
                ...
                Alert.alert(
                  "Video Deleted",
                  "The video has been removed from your device."
                );
              } catch (error) {
                Alert.alert(
                  "Delete Failed",
  ...
  const handlePlayVideo = useCallback(
    async (item: Video | DownloadedVideo) => {
      try {
              ...
              Alert.alert(
                ...
                [
                  { text: "OK" },
                  {
                    ...
                    onPress: () => dispatch(deleteVideo(item.id)),
                  },
                ]
              );
              return;
  ...
  const getFilteredVideos = useCallback(
    (videoList: Video[] | DownloadedVideo[]) => {
      let filtered = videoList;

      // Apply search filter
      if (searchQuery.trim()) {
        filtered = videoList.filter(
          (video) =>
            video.title
              .toLowerCase()
        ...
        switch (sortBy) {
          ...
          case "date":
            if ("downloadDate" in a && "downloadDate" in b) {
              return (
                new Date(b.downloadDate || 0).getTime() -
                new Date(a.downloadDate || 0).getTime()
              );
            }
            ...
            <View className="flex-row items-center justify-between mt-1">
              ...
              {isDownloading ? (
                <View className="flex-row items-center">
                  <Text className="text-[#552BFF] text-xs mr-1">
                ...
                >
                  <MaterialIcons
                    ...
                    size={14}
                    color={
                      isOffline || item.isLive ? "#9CA3AF" : "#552BFF"
                    }
                  />
                </TouchableOpacity>
              )}
  ...
  const renderDownloadedVideoItem = useCallback(
    ({ item }: { item: DownloadedVideo }) => (
      <TouchableOpacity
  ...
  const filteredVideos = getFilteredVideos(
    activeTab === "online" ? videos : downloadedVideos
  );
      ...
      <View className="px-4 pt-6 pb-4 bg-white border-b border-gray-100">
        ...
        <View className="flex-row bg-gray-100 rounded-xl overflow-hidden">
          ...
          >
            <Text
              className={`font-medium ${
                activeTab === "offline" ? "text-white" : "text-gray-600"
              }`}
            >
